...
//if(!$query)return;
$tags = array('beliebt'=>__('Die Beliebtesten'),'schoen'=>__('Die Schönsten'),'bedeutend'=>__('Die Bedeutendsten'),'zufaellig'=>__('Zufällig'));
?>
<div id="archiveWrapper" class="searchResults">
 <div class="header">
<div class="searchResultsHeader">
      <?php echo $doc_total; ?> Suchergebnisse zu Stöbern "<?php echo $tags[$query]; ?>"
    </div>
    <div class="searchResultFilter">
      <form method="get" id="searchResultFilter" action="<?php echo url_for('lesesaal_search');?>" onchange="document.getElementById('searchResultFilter').submit();">
        ...
        <?php foreach(array(16,32,48,64,96) as $i) :
        ...
        <?php foreach(array("date"=>"Alter", "name"=>"Name", "bestand_sig"=>"Systematik") as $v=>$n) :
        $archive_search_sort = $sf_user->getAttribute('archive_search_sort', null);
        $selected = ($archive_search_sort==$v)? ' selected="selected"' : '' ?>
          <option value="<?php echo $v; ?>"<?php echo $selected; ?>><?php echo __('nach'); ?> <?php echo $n; ?></option>
        <?php endforeach; ?>
        </select>
  ...
  <div class="content">
    ...
    <?php if ($doc_pager AND $doc_pager->haveToPaginate()): ?>
      <div class="paging">
        <a href="<?php echo url_for('stoebern', array('tags'=>$query, 'page'=>$doc_pager->getPreviousPage())); ?>" class="prev"><?php echo __('Seite zurück') ?></a>
        <div class="pages">
        <?php foreach ($doc_pager->getLinks() as $page): ?>
          <?php $active = ($page == $doc_pager->getPage())? ' class="active"' : ''; ?>
            <a href="<?php echo url_for('stoebern', array('tags'=>$query, 'page'=>$page)); ?>"<?php echo $active ?>><?php echo $page ?></a>
        <?php endforeach; ?>
        </div>
        <a href="<?php echo url_for('stoebern', array('tags'=>$query, 'page'=>$doc_pager->getNextPage())); ?>" class="next"><?php echo __('Nächste Seite') ?></a>
      </div>
    <?php endif; ?>
