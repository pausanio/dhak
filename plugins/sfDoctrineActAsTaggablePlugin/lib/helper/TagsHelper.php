...
function tag_cloud($tags, $route, $options = array())
{
  ...
  if (count($tags) > 0) {
    ...
    foreach ($tags as $name => $count) {
      ...
      /**
       * @bugfix 'Notice: Undefined index: -0'
       * @author  2012-12-05, Maik Mettenheimer <mettenheimer@pausanio.de>
       */
      if ($count == -0) {
        $count = 0;
      }
...
function tag_list($tags, $route, $options = array())
{
  ...
  if (count($tags) > 0) {
    ...
    foreach ($tags as $tag) {
      $link = link_to($tag, $route . $tag, array('rel' => 'tag'));
...
function related_tag_cloud($tags, $route, $tag, $options = array())
{
  ...
  if (count($tags) > 0) {
    ...
    foreach ($tags as $name => $count) {
      $link = link_to($name, $route . $name, array('rel' => 'tag'));
      $related_link = link_to($add, $route . $tag . ',' . $name);
