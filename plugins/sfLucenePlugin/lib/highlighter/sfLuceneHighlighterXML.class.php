...
class sfLuceneHighlighterXML extends sfLuceneHighlighter
{
  ...
  protected function doHighlightTextNode(DOMNode $node)
  {
    foreach ($this->tokenize($node->textContent) as $token)
    {
      ...
      #ivoba: umlaut hack: splitText scheint Problem mit umlauten zu haben
      $umlauts = 0;
      $umlauts += substr_count($node->nodeValue, 'ü');
      $umlauts += substr_count($node->nodeValue, 'ö');
      $umlauts += substr_count($node->nodeValue, 'ä');
      $umlauts += substr_count($node->nodeValue, 'Ü');
      $umlauts += substr_count($node->nodeValue, 'Ö');
      $umlauts += substr_count($node->nodeValue, 'Ä');
      $umlauts += substr_count($node->nodeValue, 'ß');
      $matched = $node->splitText($token->getStart()-$umlauts);
